...
public interface DocumentRepository extends JpaRepository<Document, Long> {
    ...
    @Query(
        value = "select document from Document document left join fetch document.customer",
        countQuery = "select count(document) from Document document"
    )
    Page<Document> findAllWithToOneRelationships(Pageable pageable);

    @Query("select document from Document document left join fetch document.customer")
    List<Document> findAllWithToOneRelationships();

    @Query("select document from Document document left join fetch document.customer where document.id =:id")
