...
public class UserService {
    ...
    private String generateOtp() {
        return String.valueOf((int) (Math.random() * 900000) + 100000); // رقم مكون من 6 أرقام
    }

    public User registerUser(UserDTO userDTO) {
        // إنشاء مستخدم جديد بناءً على UserDTO
        User user = new User();
        user.setLogin(userDTO.getLogin().toLowerCase());
        user.setEmail(userDTO.getEmail().toLowerCase());
        user.setActivated(false); // المستخدم غير مفعل افتراضيًا
        user.setPassword(passwordEncoder.encode(RandomUtil.generatePassword())); // كلمة مرور عشوائية
        ...
        log.debug("Sent OTP {} to user {}", otp, user.getLogin());
